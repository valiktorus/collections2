...
import by.gsu.epamlab.comparators.DescendingNumberComparator1;

import java.io.FileNotFoundException;
import java.io.FileReader;
import java.util.*;

public class TreeSetRunner3 {
    public static void main(String[] args) {
        try(Scanner scanner = new Scanner(new FileReader(Constants.FILE_NAME))){
            NavigableSet<NumLen1> sortedSetNumLen = new TreeSet<>();
            List<Integer> array = ArrayCreator.getE();
            long start = System.currentTimeMillis();
            for (int length: array) {
                NumLen1 numLen1 = new NumLen1(length);
                sortedSetNumLen.add(numLen1);
            }
            List<NumLen1> list = new ArrayList<>(sortedSetNumLen);
            Collections.sort(list, new DescendingNumberComparator1());
